...
def scrape_chapter(mech_browser, url, main_content_style='verses'):
    ...
    if main_content:
        ...
        # First strip out the verse numbers
        ...
        if verse_markers:
            for v in verse_markers:
                v.drop_tree() #get rid of verse number, but keep tail (the first few words of the verse)
                    ...
                    # "text" is "text here"
                    # "tail" is " more text"
        # Now strip out the footnote indicators (e.g. 'a', 'b', etc)
        ...
        # Now start counting the verses and words
        num_verses = 0
        verse_word_lens = []
        verse_texts = []
...
def scrape_book(mech_browser, book_name, url_name_list, sleep=SLEEP, verbose=False):
    ...
    Return a list of 2-element lists as follows
    [ [ info_dict, [list of chapter info] ],
      [ info_dict, [list of chapter info] ],
    ]
    In more detail, the info_dict and chapter information looks as follows:
    [
    ...
    for url, name, main_content_style in url_name_list:
        ...
        chapter_results.append( [{'name':name, 'verses':num_verses, 'words':num_words}, verse_lens, verse_texts] )
        time.sleep(sleep+random.uniform(-1*sleep/2.0, sleep/2.0))
    if verbose: print ''
    num_chapters = len(chapter_results)
    num_verses = sum( [c[0]['verses'] for c in chapter_results] )
    num_words = sum( [c[0]['words'] for c in chapter_results] )
    info_dict = {'name':book_name, 'chapters':num_chapters, 'verses':num_verses, 'words':num_words}
    return [info_dict, chapter_results]
...
def scrape_scripture(script_name, url_base, books, sleep=SLEEP, verbose=False):
    ...
    format:
    [
      [info_dict, [list of book info] ],
      [info_dict, [list of book info] ], ...
    ]
    The info_dict is a dictionary with keys 'name', 'books', 'chapters', 'verses', 'words'.
    See docstring for scrape_book() for the format of book info
    ...
    for book, book_nickname, num_chaps in books:
        ...
        url_and_names = []
    num_books = len(books)
    num_chapters = sum( [b[0]['chapters'] for b in book_results] )
    num_verses = sum( [b[0]['verses'] for b in book_results] )
    num_words = sum( [b[0]['words'] for b in book_results] )
    info_dict = {'name':script_name, 'url': url_base, 'books':num_books,
                 'chapters':num_chapters, 'verses':num_verses, 'words':num_words}
    return [info_dict, book_results]
...
def simplify_info(scriptures=None):
    ...
    for fname in scriptures:
        ...
        script_info = data[0]
        book_list = data[1]
        print '%s: read item with %d entries.  Each entry has %d items' % (script_info['name'],
                                                                           len(book_list),
                                                                           len(book_list[0]))
        simple_book_list = []
        for book in book_list:
            ...
            for chapter in chapter_list:
                ...
                simple_chapter_list.append(chapter_info) # keep just the info dict
            simple_book_list.append( [book_info, simple_chapter_list] )
        all_scripts.append( [script_info, simple_book_list] )
    try:
        f = open('all_scripts_basic_info.json', 'w')
        f.write(json.dumps(all_scripts))
    finally:
        f.close()
    return all_scripts

def one_time_fix():
    scriptures = ['dc-testament', 'bofm', 'old pgp', 'ot', 'nt']
    all_scripts = []
    for fname in scriptures:
        try:
            f = open(fname+'_info.json', 'r')
        ...
        for book in book_list:
            ...
            for chapter in chapter_list:
                ...
                chapter_verses = chapter[1]
                chapter_words = chapter[2]
                ch_info_dict = {'name':chapter_name, 'verses':chapter_verses, 'words':chapter_words}
                new_chapter_list.append( [ch_info_dict, chapter[3], chapter[4]] )
            b_info_dict = {'name':book_name, 'chapters':book_chapters, 'verses':book_verses,
                           'words':book_words}
            new_book_list.append( [b_info_dict, new_chapter_list] )
        s_info_dict = {'name':scripture_name, 'url':url, 'books':total_books, 'chapters':total_chapters,
                       'verses':total_verses, 'words':total_words}
        try:
            f2 = open('new_'+fname+'_info.json', 'w')
            f2.write(json.dumps([s_info_dict, new_book_list]))
        except:
            print 'Error writing new file for %s' % fname
            return
        finally:
            f2.close()
        all_scripts.append( [s_info_dict, new_book_list] )
    return all_scripts
